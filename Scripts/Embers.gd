...
@export_range(20.0,  800.0, 1.0)   var source_temp:   float  = 445.0
@export_range(0.0,   300.0, 1.0)   var source_smoke:  float  = 62.0
...
@export_range(0.1,     5.0, 0.1)   var flicker_speed:    float = 1.0
...
@export_range(0.0,   50.0,  0.01)  var vorticity_fire:       float  = 10.0
@export_range(0.0,   20.0,  0.01)  var vorticity_smoke: float  = 2.0
...
@export_range(0.90,  1.0,   0.0001)var smoke_dissipation:    float = 0.988
...
@export_range(0.0,   5.0,   0.01)  var smoke_glow:          float  = 1.45
...
@export_range(1, 8192, 1) var ember_max_particles: int = 200
## Spawn amount — percentage of max_particles to keep alive (0=none, 1=all)
@export_range(0.0,   1.0,  0.01)  var ember_spawn_amount: float = 0.5
## Min lifetime in seconds
@export_range(0.5,   5.0,  0.1)  var ember_min_lifetime: float = 1.0
## Max lifetime in seconds
@export_range(0.5,   8.0,  0.1)  var ember_max_lifetime: float = 3.0
## How strongly embers follow fluid velocity
@export_range(0.0,  10.0,  0.01) var ember_vel_follow: float = 2.0
## Sideways spread strength
@export_range(0.0,  10.0,  0.01) var ember_spread_x: float = 1.0
## Gravity — negative = up
@export_range(-1200.0, 1200.0, 1.0) var ember_gravity: float = 30.0
## Drag
@export_range(0.0,   5.0,  0.01) var ember_drag: float = 1.5
## Ember size in pixels
@export_range(0.5,  10.0,  0.1)  var ember_size: float = 3.0
...
@export_range(0.0, 1.0, 0.01) var ember_hot_duration: float = 0.4
## Hot core color
@export var ember_color_hot: Color = Color(1.0, 0.95, 0.6)
## Mid flame color
@export var ember_color_mid: Color = Color(1.0, 0.35, 0.05)
## Smoke/cool color
@export var ember_color_smoke: Color = Color(0.15, 0.12, 0.10)
...
func _ember_setup() -> void:
	...
	fmt.width  = simulation_resolution
	fmt.height = simulation_resolution
...
func _ember_process(delta: float) -> void:
	...
	var clear_data := PackedByteArray(); clear_data.resize(simulation_resolution * simulation_resolution * 4)
	...
	pc_rnd.encode_s32(0,   simulation_resolution)
	...
	pc_rnd.encode_float(20, 0.0)
	pc_rnd.encode_float(24, 0.0)
	pc_rnd.encode_float(28, 0.0)
	pc_rnd.encode_float(32, ember_color_hot.r)
	pc_rnd.encode_float(36, ember_color_hot.g)
	pc_rnd.encode_float(40, ember_color_hot.b)
	pc_rnd.encode_float(44, ember_color_mid.r)
	pc_rnd.encode_float(48, ember_color_mid.g)
	pc_rnd.encode_float(52, ember_color_mid.b)
	pc_rnd.encode_float(56, ember_color_smoke.r)
	pc_rnd.encode_float(60, ember_color_smoke.g)
	pc_rnd.encode_float(64, ember_color_smoke.b)
